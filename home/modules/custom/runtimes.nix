...
let
  ...
  getMajorMinorVersion = pkg: lib.replaceStrings [ "." ] [ "_" ] (lib.versions.majorMinor (lib.getVersion pkg));
in
{
  ###### interface
  options = with lib; {
  ...
  config = lib.mkIf cfg.enable {
    ...
    home =
      let
        javaPkgs = lib.foldl' (acc: pkg: acc // { "${getMajorVersion pkg}" = pkg; }) { } (lib.unique cfg.java);
        javaEnvVariable = lib.mapAttrs' (version: pkg: lib.nameValuePair "JAVA_HOME_${version}" "${pkg.home}") javaPkgs;
        javaAliases = lib.mapAttrs' (version: pkg: lib.nameValuePair "java_${version}" "${lib.getExe pkg}") javaPkgs;
